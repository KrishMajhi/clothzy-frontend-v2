    ...
    <div className="item-rating">
      <div className="stars">
        {Array.from({ length: 5 }, (_, i) => (
          <svg key={i} className={`star ${i >= fullStars ? "empty" : ""}`} viewBox="0 0 12 12">
            <polygon points="6,1 7.5,4.5 11,4.5 8.5,7 9.5,11 6,9 2.5,11 3.5,7 1,4.5 4.5,4.5" />
          </svg>
        ))}
      </div>
      <span className="rating-text">{rating} ({review_count} reviews)</span>
    </div>
  );
};
    ...
    >
      ...
      <div className="cart-item-image">
        ...
        {item.image ? (
          ...
          <svg viewBox="0 0 148 192" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <linearGradient id={`g${item.cart_id}`} x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <text x="74" y="100" textAnchor="middle" dy=".35em" fontSize="52" fontFamily="serif">
              👕
            </text>
          </svg>
        )}
      ...
      <div className="cart-item-details">
        ...
        <div className="item-meta">
          {item.brand && (
            <span className="meta-tag">
              <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                <circle cx="5" cy="5" r="4" stroke="#b8a98e" strokeWidth="1.5" />
          ...
          {item.material && (
            <span className="meta-tag">
              <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                <path d="M2 8c0-2 1.5-4 3-4s3 2 3 4" stroke="#b8a98e" strokeWidth="1.2" />
                <ellipse cx="5" cy="3.5" rx="2" ry="1.5" stroke="#b8a98e" strokeWidth="1.2" />
              </svg>
              {item.material}
            </span>
          )}
        ...
        <div style={{ display: "flex", gap: "14px", alignItems: "center" }}>
          <span className={`stock-indicator ${item.stock > 0 ? "in-stock" : "low-stock"}`}>
            <span className={`stock-dot ${item.stock > 0 ? "in-stock" : "low-stock"}`}></span>
            <span className="stock-text">{item.stock > 0 ? "In Stock" : "Out of Stock"}</span>
          </span>
          <span className="save-later" onClick={() => onSaveForLater(item.cart_id)}>
            <svg width="11" height="11" viewBox="0 0 11 11" fill="none">
              <path d="M1 1h9v9L5.5 7 1 10V1z" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round" />
            </svg>
            Save for later
          </span>
        </div>
      ...
      <div className="item-price-section">
        <div className="price-stack">
          ...
          {hasDiscount && (
            <>
              <div className="item-original-price">${item.original_price.toFixed(2)}</div>
        ...
        <div className="qty-wrap">
          <div className="qty-control">
            <button className="qty-btn" onClick={() => onUpdateQty(item.cart_id, -1)}>−</button>
