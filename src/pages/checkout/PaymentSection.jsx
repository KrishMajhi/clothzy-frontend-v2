...
const cardDisplay   = (v) => { const c = v.replace(/\s/g, ""); let d = ""; for (let i = 0; i < 16; i++) { if (i > 0 && i % 4 === 0) d += " "; d += i < c.length ? c[i] : "•"; } return d; };
    ...
    <div className="co-card-preview">
      ...
      <div className="co-card-bottom">
        <div>
          ...
          <div className="co-card-val-sm">{card.name.toUpperCase() || "FULL NAME"}</div>
        </div>
        <div>
  ...
  return (
    <div className="co-card">
      ...
      <div className="co-pay-tabs">
        {PAY_METHODS.map(m => (
          <button key={m.id} className={`co-pay-tab${payMethod === m.id ? " co-pay-tab--active" : ""}`}
            onClick={() => onPayMethodChange(m.id)}>
            <span className="co-pay-tab-icon">{m.icon}</span>{m.label}
          </button>
        ))}
      </div>
      ...
      {payMethod === "card" && (
        <div className="co-pay-panel">
          ...
          <div className="co-row">
            <Field label="Name on Card" placeholder="Arjun Sharma"
              value={card.name} onChange={v => setCard(p => ({ ...p, name: v }))} />
          </div>
          <div className="co-row">
            <Field label="Card Number" placeholder="1234 5678 9012 3456"
              value={card.number} onChange={v => setCard(p => ({ ...p, number: formatCardNum(v) }))} maxLength={19} />
          </div>
          <div className="co-row co-row--2">
            <Field label="Expiry Date" placeholder="MM / YY"
              value={card.expiry} onChange={v => setCard(p => ({ ...p, expiry: formatExp(v) }))} maxLength={7} />
            <Field label="CVV" type="password" placeholder="•••"
              value={card.cvv} onChange={v => setCard(p => ({ ...p, cvv: v.replace(/\D/g, "").slice(0, 4) }))} maxLength={4} />
          </div>
          <div className="co-accepted-cards">
            <span className="co-accepted-label">Accepted:</span>
            {["Visa","Mastercard","RuPay","Amex"].map(c => <span key={c} className="co-card-pill">{c}</span>)}
          </div>
        </div>
      )}
        ...
        <div className="co-pay-panel">
          <div className="co-row">
            <div className="co-field-group">
              ...
              <input className="co-field-input" placeholder="yourname@upi"
                value={upiId} onChange={e => setUpiId(e.target.value)} />
            </div>
          </div>
          <p className="co-field-label" style={{ margin: "16px 0 10px" }}>Or pay with</p>
          <div className="co-upi-apps">
            {UPI_APPS.map(a => (
              <button key={a.name} className={`co-upi-btn${upiApp === a.name ? " co-upi-btn--active" : ""}`}
      ...
      {payMethod === "cod" && (
        <div className="co-cod-notice">
          ...
          <div>
            ...
            <div className="co-cod-desc">Pay in cash when your order arrives. A ₹49 handling fee applies. Our delivery partner will collect the exact amount upon delivery.</div>
          </div>
        </div>
      )}
    </div>
